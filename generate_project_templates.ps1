# generate_project_templates.ps1
# Script to create control.html and dashboard.html for each folder that starts with "project-"
# Uses a generic template with MQTT, sensors, alerts, localStorage and Chart.js

$basePath = (Get-Location).Path
$projectDirs = Get-ChildItem -Path $basePath -Directory -Filter "project-*"

# Template strings (PowerShell here-strings) with placeholder __PROJECT_ID__
$controlTemplate = @'
<!DOCTYPE html>
<html lang="es">
<head>
...
foreach ($dir in $projectDirs) {
    ...
    Write-Host "Created $controlPath and $dashboardPath"
}
